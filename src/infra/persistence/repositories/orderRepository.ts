...
export class OrderRepository implements IOrderRepository {
  async createOrder(orderNew: Order): Promise<Order> {
    return new Promise<Order>(async (resolve) => {
      ...
      const codeNew = `ORD-${codeNumber}Se conectar no microservico de pagamento para obter o Id do pagamento`;

      let orderCreated = await OrderModel.create({
        statusId: orderStatusModel?.id,
        basketId: basketModel?.id,
        paymentId:
          "Se conectar no microservico de pagamento para obter o Id do pagamento",
